...
-- Latest 10 uploads:
select * from (select u.upload_id,u.name,(content_length/1024) as size_kb,strftime('%Y-%m-%d %H:%M:%S', datetime(created_on/1000,'unixepoch')) created_on,ct.content_type,ips.ip from uploads u join content_types ct on u.content_type_id=ct.content_type_id join ips on u.ip_id=ips.ip_id order by created_on desc limit 10) order by created_on;
```
